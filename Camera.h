...
//	int count = 0;
	...
	FPCamera( glm::vec3 position = glm::vec3(0.0f, 0.0f , 0.0f), glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f), float y = YAW, float p = PITCH) : Front(0.0f, 0.0f, -1.0f ), speed(SPEED), sensitivity(SENSITIVITY), zoom(ZOOM){
		Position = position;
		WorldUp = up;
		yaw = y;
		pitch = p;
	...
	FPCamera(float posX, float posY, float posZ, float upX, float upY, float upZ, float y = YAW, float p = PITCH) : Front(0.0f, 0.0f, -1.0f), speed(SPEED), sensitivity(SENSITIVITY), zoom(ZOOM) {
		Position.x = posX;
		Position.y = posY;
		Position.z = posZ;
		WorldUp.x = upX;
